					  ...
					  dma_filter_fn fn, void *fn_param)
{
	...
	list_for_each_entry(chan, &dev->channels, device_node) {	//Iterate through channels of dma device.
//printk("drivers/dma/dmaengine.c:private_candidate: name = %s\n", chan->name);
		if (chan->client_count) {
//printk("drivers/dma/dmaengine.c:private_candidate: Client count = %d\n", chan->client_count);
			dev_dbg(dev->dev, "%s: %s busy\n",
				 __func__, dma_chan_name(chan));
			continue;
		}
//printk("drivers/dma/dmaengine.c:private_candidate: Filter function START\n");
		if (fn && !fn(chan, fn_param)) {
			dev_dbg(dev->dev, "%s: %s filter said false\n",
				 __func__, dma_chan_name(chan));
//printk("drivers/dma/dmaengine.c:private_candidate: Filter function END\n");
			continue;
		}
//printk("drivers/dma/dmaengine.c:private_candidate: Filter function END\n");
		return chan;
				       ...
				       struct device_node *np)
{
	...
	/* Find a channel */
	mutex_lock(&dma_list_mutex);
	list_for_each_entry_safe(device, _d, &dma_device_list, global_node) {	//Iterate dma_device_list
		/* Finds a DMA controller with matching device node */
		if (np && device->dev->of_node && np != device->dev->of_node) {
printk("drivers/dma/dmaengine.c:__dma_request_channel SHOULD NOT HAPPEN!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
			continue;
}
...
struct dma_chan *dma_request_chan(struct device *dev, const char *name)
{
	...
	struct dma_chan *chan = NULL;
//printk("drivers/dma/dmaengine.c:dma_request_chan: dev name = %s, name = %s\n", dev->kobj.name , name);
	/* If device-tree is present get slave info from here */
	if (dev->of_node)
		chan = of_dma_request_slave_channel(dev->of_node, name);	//Here comes chan.

	/* If device was enumerated by ACPI get slave info from here */
	if (has_acpi_companion(dev) && !chan)	//False
		chan = acpi_dma_request_slave_chan_by_name(dev, name);

	if (PTR_ERR(chan) == -EPROBE_DEFER)
		return chan;

	if (!IS_ERR_OR_NULL(chan))	//True
		goto found;
...
found:
	...
	if (sysfs_create_link(&dev->kobj, &chan->dev->device.kobj, chan->name))
		dev_warn(dev, "Cannot create DMA %s symlink\n", chan->name);
//printk("drivers/dma/dmaengine.c:dma_request_chan: dev name = %s, name = %s END\n", dev->kobj.name , name);
	return chan;
}
