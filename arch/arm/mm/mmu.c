			...
			const struct mem_type *type, bool ng)
{
	...
	do {
#ifdef CONFIG_TRUSTFULL_HYPERVISOR
		HYPERCALL_3(HYPERCALL_CREATE_SECTION, addr, phys, type->prot_sect | (ng ? PMD_SECT_nG : 0));
#else
...
static __init void prepare_page_table(void)
{
...
#else
#ifdef CONFIG_TRUSTFULL_HYPERVISOR
	HYPERCALL_2(1055, 0, MODULES_VADDR);
#else
...
#ifdef CONFIG_TRUSTFULL_HYPERVISOR
	HYPERCALL_2(1055, MODULES_VADDR, PAGE_OFFSET);
#else
	 ...
	 */
#ifdef CONFIG_TRUSTFULL_HYPERVISOR
	HYPERCALL_2(1055, __phys_to_virt(end), VMALLOC_START);
#else
...
static void __init devicemaps_init(const struct machine_desc *mdesc)
{
	struct map_desc map;
	unsigned long addr;
#ifndef CONFIG_TRUSTFULL_HYPERVISOR	//Hypervisor sets up exception vector.
	void *vectors;
	 ...
	 */
#ifdef CONFIG_TRUSTFULL_HYPERVISOR
	unsigned int start_va = VMALLOC_START;
	//The last 256MB of the virtual address space is reserved for the
	//hypervisor.
	unsigned int end_va = FIXADDR_TOP & PMD_MASK & 0xF0000000;
	end_va = end_va >= 0xF0000000 ? 0xF0000000 : end_va;
	HYPERCALL_2(1055, start_va, end_va);
#else
...
#ifndef CONFIG_TRUSTFULL_HYPERVISOR	//TLBs and caches are flushed/invalidated by the hypervisor in each hypercall.
	/*
	...
	//Executes CPSIE a instruction: "Change Processor State" (CPS) "Interrupt or
	//abort enable" (IE) "Enables or disables imprecise aborts" (a).
	early_abt_enable();

#ifdef CONFIG_TRUSTFULL_HYPERVISOR
	//Initializes hypervisor exception vector data structures, now that the
	//corresponding tasks are completed in Linux.
#define HYPERCALL_GUEST_INIT 1000
	extern uint32_t __hypervisor_vector[];
	//NR_syscalls is defined to 452 in arch/arm/include/generated/asm/unistd-nr.h.
	HYPERCALL_1(HYPERCALL_GUEST_INIT, /*NR_syscalls,*/ __hypervisor_vector);
#endif
//#ifdef CONFIG_TRUSTFULL_HYPERVISOR
//	extern uint32_t __hypervisor_vector[];
//	HYPERCALL_1(HYPERCALL_GUEST_INIT, __hypervisor_vector);
//#endif
}
...
#ifndef CONFIG_TRUSTFULL_HYPERVISOR
	//Clearing one PMD corresponds to clearing 2MB
//	HYPERCALL_2(1055, va & 0xFFE00000, (va & 0xFFE00000) + 0x00200000);
	//Done by hypervisor in pmd_clear.
