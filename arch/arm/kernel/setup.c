...
static int __get_cpu_architecture(void)
{
	...
	} else if ((read_cpuid_id() & 0x000f0000) == 0x000f0000) {	//TRUE
		/* Revised CPUID format. Read the Memory Model Feature
		 * Register 0 and check for VMSAv7 or PMSAv7 */
		unsigned int mmfr0 = read_cpuid_ext(CPUID_EXT_MMFR0);

		if ((mmfr0 & 0x0000000f) >= 0x00000003 ||			//TRUE
...
void notrace cpu_init(void)
{
	 ...
	 */
#ifdef CONFIG_THUMB2_KERNEL	//FALSE
#define PLC_l	"l"
#define PLC_r	"r"
#else						//TRUE
#define PLC_l	"I"
...
#ifdef MULTI_TLB	//FALSE
	cpu_tlb = *list->tlb;
#endif
#ifdef MULTI_USER	//TRUE since CONFIG_CPU_COPY_V6
	cpu_user = *list->user;
#endif
#ifdef MULTI_CACHE	//FALSE
...
#ifndef CONFIG_ARM_THUMB						//TRUE
	elf_hwcap &= ~(HWCAP_THUMB | HWCAP_IDIVT);
#endif
#ifdef CONFIG_MMU								//TRUE
...
static int __init init_machine_late(void)
{
	...
	int ret;
//printk("arch/arm/kernel/setup.c:init_machine_late START\n");
	...
	if (!system_serial)
		system_serial = kasprintf(GFP_KERNEL, "%08x%08x",
					  system_serial_high,
					  system_serial_low);
//printk("arch/arm/kernel/setup.c:init_machine_late END\n");
	return 0;
}
...
//CONFIG_LIBFDT makes lib/Makefile instruct the compiler to compile lib/fdt.c,
//which includes the line #include "../scripts/dtc/libfdt/fdt.c", which contains
//the function that fails, and is thus compiled both for the native development
//host (to be able to compile the dts) and for the kernel image.
	if (__atags_pointer)
